...
if(open(F,'.config')) {
    ...
    $need_tests=$d{test_dsn} eq 'none' ? 0 : 1;
}
if(!$need_tests) {
    print <<EOT;

Skipping tests because no test database given. Please remove '.config'
file and re-run Makefile.PL to enable tests.
...
of what you think might be the reason.
===============================================================
END_OF_WARNING
